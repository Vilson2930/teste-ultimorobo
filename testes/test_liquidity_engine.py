...
def make_access_map(tmp_path):
    df = pd.DataFrame(
        {
            "ativo": ["BTC-USD", "USDT-USD", "GLD", "VOO", "TLT"],
            "trilho": ["SELF_CUSTODY", "EXCHANGE", "ETF", "ETF", "ETF"],
            "entidade": [
                "SELF_CUSTODY",
                "TETHER",
                "STATE_STREET",
            ...
            "jurisdicao": ["GLOBAL", "GLOBAL", "USA", "USA", "USA"],
            "status": ["ATIVO", "ATIVO", "ATIVO", "ATIVO", "ATIVO"],
            "criticidade": ["MEDIA", "MEDIA", "BAIXA", "BAIXA", "BAIXA"],
            "bucket_sobrevivencia": [True, True, True, True, True],
            "jurisdicao_valida": [True, True, True, True, True],
        }
    )
...
def test_liquidity_level_existe(tmp_path):
    ...
    assert level in ["ROBUSTO", "ACEITAVEL", "FRAGIL", "CRITICO"]


def test_haircut_agregado_existe(tmp_path):
    result = run_engine(tmp_path)

    haircut = result["liquidity_summary"].iloc[0]["aggregate_haircut_pct"]
