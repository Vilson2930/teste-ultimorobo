...
def make_rebalance():
    df = pd.DataFrame({
        ...
        "valor_atual": [40000, 7000, 16000, 17000, 7000, 7000, 6000],
        "peso_atual": [0.40, 0.07, 0.16, 0.17, 0.07, 0.07, 0.06],
    })

    return df.set_index("ativo")


def test_risk_budget_gera_saida():
    result = run_risk_budget_engine(make_rebalance(), market_data=None)
...
def test_top_risk_asset_existe():
    ...
    assert top_asset in ["BTC-USD", "USDT-USD", "GLD", "VOO", "TLT", "BOTZ", "INDA"]
